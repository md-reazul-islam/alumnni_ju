<x-layouts::admin :title="'Catering Complaints'">
    <x-breadcrumb :items="[['label' => 'Catering'], ['label' => 'Reports', 'url' => route('admin.catering.reports.index')], ['label' => 'Complaints']]" class="mb-4" />
            ...
            @foreach ($complaints as $complaint)
                @php
                    ...
                    $targetLabel = match (true) {
                        $target instanceof \App\Models\CateringOrder => "Catering Order #{$target->id}: {$target->category?->name}",
                        $target instanceof \App\Models\CateringHomemadeOrder => "Home Made Order #{$target->id}: {$target->listing?->title}",
                        default => $target?->full_name ?? 'Deleted user',
                    };
                @endphp
                <div class="card card-body">
                    ...
                    @if ($complaint->status === 'pending')
                        <div class="mt-3 flex gap-2">
                            <form method="POST" action="{{ route('admin.catering.reports.complaints.resolve', $complaint) }}">
                                @csrf
