<x-layouts::admin :title="'Order #' . $order->id">
    <x-breadcrumb :items="[['label' => 'Catering'], ['label' => 'Home Made Orders', 'url' => route('admin.catering.homemade-orders.index')], ['label' => '#' . $order->id]]" class="mb-4" />
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div class="space-y-6 lg:col-span-2">
            <div class="card card-body">
                ...
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
                    <a href="{{ route('admin.catering.homemade-listings.show', $order->listing) }}" class="hover:underline">{{ $order->listing->title }}</a>
                    &middot; ${{ number_format($order->listing->price, 2) }} &middot; Qty {{ $order->quantity }}
                </p>
            ...
            <div class="card card-body">
                ...
                <form method="POST" action="{{ route('admin.catering.homemade-orders.status', $order) }}" class="mt-4 space-y-4">
                    @csrf
                    ...
                    <x-input label="Final Price (used for commission on completion)" name="final_price" type="number" step="0.01" min="0" :value="$order->final_price ?? round($order->listing->price * $order->quantity, 2)" />
        ...
        <div class="space-y-4">
            <div class="card card-body">
                ...
                <form method="POST" action="{{ route('admin.catering.homemade-orders.converse', [$order, 'buyer']) }}" class="mt-3">
                    @csrf
                    <x-button type="submit" size="sm" class="w-full">{{ $order->buyerConversation ? 'View Buyer Thread' : 'Message Buyer' }}</x-button>
                </form>

                <form method="POST" action="{{ route('admin.catering.homemade-orders.converse', [$order, 'seller']) }}" class="mt-2">
                    @csrf
