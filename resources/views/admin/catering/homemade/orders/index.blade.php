<x-layouts::admin :title="'Home Made Food Orders'">
    <x-breadcrumb :items="[['label' => 'Catering'], ['label' => 'Home Made Orders']]" class="mb-4" />
    <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Home Made Food Orders</h1>
    ...
    @else
        <x-table class="mt-6">
            <thead><tr><th>Listing</th><th>Qty</th><th>Buyer</th><th>Seller</th><th>Status</th><th>Commission</th><th></th></tr></thead>
            <tbody>
                @foreach ($orders as $order)
                    <tr>
                        ...
                        <td><a href="{{ route('admin.catering.homemade-orders.show', $order) }}" class="text-navy-700 hover:underline dark:text-navy-300">View</a></td>
                    </tr>
                @endforeach
            </tbody>
        </x-table>
