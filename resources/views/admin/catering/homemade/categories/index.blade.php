<x-layouts::admin :title="'Home Made Food Categories'">
    <div x-data="{ adding: false, editing: null }">
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Home Made Food Categories</h1>
            <x-button size="sm" @click="adding = !adding"><x-icon name="plus" class="h-4 w-4" /> Add Category</x-button>
        </div>
        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Commission percentage is applied to completed Home Made Foods orders in this category — change it anytime.</p>

        @if (session('status'))
            <x-alert variant="success" class="mt-4">{{ session('status') }}</x-alert>
        @endif

        <form method="POST" action="{{ route('admin.catering.homemade-categories.store') }}" x-show="adding" x-cloak class="card card-body mt-4 space-y-4">
            @csrf
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                <x-input label="Category Name" name="name" required />
                <x-input label="Icon (optional)" name="icon" hint="e.g. cooking-pot, cake, soup" />
        ...
        @if ($categories->isEmpty())
            <x-empty-state icon="cooking-pot" title="No categories yet" class="mt-8" />
        @else
            <x-table class="mt-6">
                    ...
                    @foreach ($categories as $category)
                        <tr>
                            ...
                            <td class="flex items-center gap-3">
                                ...
                                <form method="POST" action="{{ route('admin.catering.homemade-categories.destroy', $category) }}" onsubmit="event.preventDefault(); Swal.fire({title:'Remove this category?',icon:'warning',showCancelButton:true,confirmButtonColor:'#dc2626',confirmButtonText:'Remove'}).then(r=>r.isConfirmed&&this.submit())">
                                    @csrf @method('DELETE')
                        ...
                        <tr x-show="editing === {{ $category->id }}" x-cloak>
                            <td colspan="5">
                                <form method="POST" action="{{ route('admin.catering.homemade-categories.update', $category) }}" class="flex flex-wrap items-end gap-4 rounded-lg bg-slate-50 p-4 dark:bg-navy-900">
                                    @csrf @method('PUT')
