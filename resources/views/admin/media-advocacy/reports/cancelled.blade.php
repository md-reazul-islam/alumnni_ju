<x-layouts::admin :title="'Cancelled Orders'">
    <x-breadcrumb :items="[['label' => 'Media Advocacy'], ['label' => 'Reports', 'url' => route('admin.media-advocacy.reports.index')], ['label' => 'Cancelled']]" class="mb-4" />
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Cancelled Orders</h1>
        <a href="{{ route('admin.media-advocacy.reports.export', 'cancelled') }}" class="btn-secondary btn-sm">Export CSV</a>
    </div>
    ...
    @else
        <x-table class="mt-6">
            <thead><tr><th>Order #</th><th>Service</th><th>Customer</th><th>Handled By</th><th>Cancelled At</th><th></th></tr></thead>
            <tbody>
                @foreach ($orders as $order)
                    <tr>
                        ...
                        <td>{{ $order->updated_at->format('M j, Y g:ia') }}</td>
