@if (! $activeConversation)
    <div class="flex flex-1 items-center justify-center p-6">
        <x-empty-state icon="message-circle" title="Select a conversation" description="Choose a conversation from the list or start a new one from an alumni profile." />
    </div>
@else
    <div class="flex items-center gap-3 border-b border-slate-200 bg-white p-4 dark:border-navy-800 dark:bg-navy-900">
        ...
        <div class="min-w-0">
            ...
            @if ($otherParticipant && Route::has('alumni.profile.show'))
                <a href="{{ route('alumni.profile.show', $otherParticipant) }}" class="text-xs text-navy-600 hover:underline dark:text-navy-300">View profile</a>
            @endif
        </div>
    </div>
    ...
    <div class="flex-1 space-y-3 overflow-y-auto p-4" id="message-scroll">
        <div id="messages-container" class="space-y-3">
            @foreach ($messages as $message)
                @include('alumni.messages.partials.bubble', ['message' => $message, 'authId' => auth()->id()])
            @endforeach
        </div>
    </div>

    <form onsubmit="AlumniMessages.sendMessage(event, {{ $activeConversation->id }})" class="flex items-center gap-2 border-t border-slate-200 bg-white p-3 dark:border-navy-800 dark:bg-navy-900">
