<x-layouts::alumni :title="'My Home Made Listings'">
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-slate-900 dark:text-white">My Home Made Listings</h1>
        <x-button :href="route('catering.homemade.create')" size="sm"><x-icon name="plus" class="h-4 w-4" /> List a Food</x-button>
    </div>
    ...
    @if ($listings->isEmpty())
        <x-empty-state icon="cooking-pot" title="You haven't listed any home made foods yet" class="mt-8" />
    @else
                    ...
                    <div class="card-body">
                        ...
                        <div class="mt-3 flex flex-wrap gap-2 text-xs text-slate-500 dark:text-slate-400">
                            @if ($listing->pending_orders_count)
                                <span class="rounded-full bg-amber-100 px-2 py-0.5 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300">{{ $listing->pending_orders_count }} new order</span>
                            @endif
                        ...
                        <div class="mt-3 flex items-center gap-3">
                            <a href="{{ route('catering.homemade.edit', $listing) }}" class="text-sm font-medium text-navy-700 hover:underline dark:text-navy-300">Edit</a>
                            <form method="POST" action="{{ route('catering.homemade.destroy', $listing) }}" onsubmit="event.preventDefault(); Swal.fire({title:'Delete this listing?',icon:'warning',showCancelButton:true,confirmButtonColor:'#dc2626',confirmButtonText:'Delete'}).then(r=>r.isConfirmed&&this.submit())">
                                @csrf @method('DELETE')
