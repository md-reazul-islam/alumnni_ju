...
<div class="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
    @foreach ($books as $book)
        <a href="{{ route('library.show', $book) }}" class="card overflow-hidden transition hover:shadow-popover">
            <div class="flex aspect-[3/4] items-center justify-center bg-gold-50 text-gold-500 dark:bg-navy-800">
                @if ($book->cover_url)
            ...
            <div class="card-body">
                ...
                @if ($book->author)
                    <p class="mt-0.5 text-xs text-slate-500 dark:text-slate-400">{{ $book->author }}</p>
                @endif
                <p class="mt-1.5 text-xs text-navy-600 dark:text-navy-300">Donated by {{ $book->donor->full_name }}</p>
            </div>
        </a>
    @endforeach
</div>
