        ...
        @else
            <div class="mt-8 grid grid-cols-1 gap-6 sm:grid-cols-3">
                @foreach ($articles as $article)
                    <div class="relative">
                        <a href="{{ route('news.show', $article) }}" class="card block overflow-hidden transition hover:shadow-popover">
                        ...
                        <div class="absolute right-3 top-3 z-10">
                            <x-share-button :url="route('news.show', $article)" :title="$article->title" label="Share this article" />
                        </div>
                    </div>
                @endforeach
            </div>
